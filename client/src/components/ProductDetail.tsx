...
export default function ProductDetail() {
  ...
  const [ratingStats, setRatingStats] = useState<
    { rating: number; totalReview: number }[]
  >([]);
  ...
  const loadMoreReviews = useCallback(async () => {
    ...
    try {
      ...
      const data_review = await ReviewAPI.getReviewProductsById(
        Number(id),
        currentPage
      );
      console.log("load more review data:", data_review.data);

      const newReviewsArray = Array.isArray(data_review.data?.data)
        ? data_review.data.data
        : [];
  ...
  const colorAttr = useMemo(
    () =>
      product?.attributes.find((a) => a.name.toLowerCase().includes("màu")) ??
      null,
    [product?.attributes]
  );
  ...
  const defaultPick = useMemo(() => {
    ...
    if (colorAttr?.attributeValue?.[0])
      obj[colorAttr.name] = colorAttr.attributeValue[0].value;
    if (sizeAttr?.attributeValue?.[0])
      obj[sizeAttr.name] = sizeAttr.attributeValue[0].value;
    return obj;
  }, [colorAttr, sizeAttr]);
      ...
      if (sizeAttr && product) {
        ...
        if (!tryVar) {
          ...
          setPick({
            ...nextPick,
            ...(firstAvailable
              ? { [sizeAttr.name]: firstAvailable.value }
              : {}),
          });
          return;
        }
    ...
    <div className="container mx-auto p-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="relative bg-muted/20 p-4">
          {discountPercent > 0 && (
            <Badge className="absolute z-10 left-3 top-3 bg-red-600 text-white rounded-none shadow-sm">
              -{discountPercent}%
            </Badge>
          )}
          <Carousel
          ...
          {product &&
            ...
            product.imageProduct.length > 1 && (
              <div
                className="mt-3 flex gap-2 overflow-x-auto p-3 scrollbar-thin"
                role="tablist"
              ...
              >
                {product.imageProduct.map((src, i) => (
                  <button
                    ...
                    className={`h-16 w-16 shrink-0 rounded-md overflow-hidden border transition-all ${
                      i === currentSlide
        ...
        <div className="p-5 md:p-6 space-y-4 max-h-[80vh]">
          ...
          <div className="flex items-center flex-wrap gap-2 text-xs">
            ...
            <Badge className="bg-yellow-100 text-yellow-800">
              {product.soldQuantity.toLocaleString()} đã bán
            </Badge>
            <Badge
              className={`${
                inStock
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2 flex-wrap" role="group">
                {colorAttr.attributeValue.map((cv) => {
                  const enabled = hasVariantWithSelection(
                    product.productVariant,
                    { ...pick, [colorAttr.name]: cv.value }
                  );
                  ...
                  return (
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        handleColorChange(cv.value, colorAttr.name)
                      }
                      disabled={!enabled}
                    >
                      {cv.value}
          ...
          <Link
            to="/cloth-size"
            className="text-xs font-normal underline hover:text-purple-400"
          >
            Hướng dẫn chọn size
          </Link>
          ...
          <div className="flex flex-col gap-3 pt-2">
            <div className="flex justify-start flex-col gap-3">
              <div className="flex flex-row items-center gap-3">
                ...
                >
                  ...
                  <input
                    ...
                    value={qty}
                    onChange={(e) =>
                      handleQuantityChange(Number(e.target.value), maxQty)
                    }
                    className="w-16 text-center h-10 border-y outline-none bg-transparent"
                  />
                  <Button
                    type="button"
                    variant="ghost"
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full">
                ...
                >
                  <ShoppingCart />{" "}
                  {variant ? "Thêm vào giỏ hàng" : "Vui lòng chọn phân loại"}
                </Button>
                <Button
                  className="flex-1 w-full sm:w-auto h-10"
                  disabled={!variant || !inStock}
            ...
            {variant && (
              <div className="text-xs text-muted-foreground">
                SKU: <span className="font-medium">{variant.sku}</span> • Tồn:{" "}
